...
var EntityFactory = (function () {
    ...
    EntityFactory.prototype.getBlueprintFromName = function (name) {
        var blueprint = Array.from(this.blueprints).find(function (x) { return x.name === name; });
        if (!blueprint) {
    ...
    EntityFactory.prototype.buildBlueprintsFromTemplates = function (blueprintTemplates) {
        var _this = this;
        return new Set(Array.from(blueprintTemplates, function (x) { return new blueprint_1.BlueprintClass({
            name: x.name,
    ...
    EntityFactory.prototype.validateBlueprints = function (blueprints) {
        if (Array.from(blueprints).some(function (b) { return !b.name || b.name.length < 1; })) {
            throw new Error('All blueprints must have a name.');
        }
        if (Array.from(blueprints).some(function (b) { return !b.components || b.components.length === 0; })) {
